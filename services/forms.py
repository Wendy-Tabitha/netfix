...
class CreateNewService(forms.Form):
    ...
    description = forms.CharField(
        widget=forms.Textarea(
            attrs={"class": "form-control", "placeholder": "Enter service description"}
        )
    )
    price_hour = forms.DecimalField(
        ...
        widget=forms.NumberInput(
            attrs={"class": "form-control", "placeholder": "Enter price per hour"}
        ),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        if company_field == "All in One":
            ...
            self.fields["field"].choices = [("", "Select a service field")] + list(
                Service.FIELD_CHOICES
            )
        else:
